...
bool cmp(Orden& a, Orden& b) {
    ...
    if (a.prioridad==b.prioridad) {
        ...
        return a.prioridad<b.prioridad; //en forma ASC
    }
}

void crear_RCL(vector<Orden> &RCL, int prioridad_mayor, vector<Orden> &ordenes) {
    for (int i=0;i<ordenes.size();i++) {
        if (ordenes[i].prioridad==prioridad_mayor) {
...
void hallar_solucion(vector<Linea> &lineas_iteracion, vector<Orden> ordenes, int &makespan_iteracion) {
    while (!ordenes.empty()) {
        ...
        int prioridad_mayor=ordenes[0].prioridad;
        vector<Orden> RCL;
        crear_RCL(RCL, prioridad_mayor, ordenes);
