...
// ----------------------------------------------------------
// Aberración: Valida restricciones estrictas del problema
// Devuelve true si la solución rompe alguna regla fundamental.
// ----------------------------------------------------------
bool aberracion(const vector<int> &ind,
    ...
    for (int t = 0; t < numCam; t++) {
        ...
        if (uso > camiones[t]) return true; // Excede capacidad máxima
    }

    // 2) Cada paquete debe estar asignado EXACTAMENTE a un camión
    for (int p = 0; p < numPaq; p++) {
        int cuenta = 0;
        for (int t = 0; t < numCam; t++) {
            int idx = t * numPaq + p;
        ...
        // Aberración si no está asignado (cuenta == 0) o si está en más de uno (cuenta > 1)
        if (cuenta != 1) return true;
    }

    return false; // Individuo válido
}
...
int muestramejor(const vector<vector<int>> &poblacion,
    ...
    cout << endl << "La mejor solucion tiene fitness (Lexicografico): "
         << mejorFit << endl;
    cout << "Cromosoma: ";
    ...
    for (int t = 0; t < numCam; t++) {
        ...
        sumaCap += camiones[t];
        cout << "  [uso=" << uso << ", desperdicio=" << (uso > 0 ? desp : 0) << "]" << endl;
    }

    cout << "\nRESUMEN DE LA MEJOR SOLUCIÓN:" << endl;
    cout << "-> CAMIONES TOTALES UTILIZADOS = " << camionesContados << " de " << numCam << endl;
    cout << "-> DESPERDICIO EN CAMIONES USADOS = " << desperdicioTotal << " kg." << endl;
...
int main() {
    // Ejemplo TUPIACU (Corregido según comentarios):
    // 6 paquetes
    // Pesos: 150, 100, 180, 50, 120, 10
    int paq[] = {15, 32, 20, 42, 8, 18};
    int numPaq = sizeof(paq)/sizeof(paq[0]);

    // 4 camiones
    // Capacidades: 250, 200, 200, 100
    int camiones[] = {50,50,50,50};
